...
folder="FIXED/"
val=""
validation_approach="Fixed"

# split training and test data, with random seed 220516
mkdir "$folder""$validation_approach""Train_Data"
mkdir "$folder""$validation_approach""Test_Data"
mkdir "$folder""$validation_approach""Trained_Models"
python3 train_test_separation.py "$folder" 220516 "$validation_approach"
...
do
	...
	python3 scaling.py "$folder" "$var" "$validation_approach"
	for model in linear_regression.py logistic_regression.py elastic_net.py random_forest.py XGBoost.py SVM.py ANN.py
	do 
		python3 $model "$folder" "$var" "$val" "$validation_approach" 1
	done
	python3 ensemble.py "$folder" "$var" "$val" "$validation_approach"
done
...
do
	...
	do
		...
		do
			python3 evaluate.py $model $trainset "Prevalence" 0.95 "$folder" "$var" "$val" "$validation_approach" >> "$folder""$var""$validation_approach""evaluate.csv"
		done
	done
	...
	do 
		...
		do
			...
			do
				python3 evaluate.py $model $trainset "$outcome" 0.95 "$folder" "$var" "$val" "$validation_approach" >> "$folder""$var""$validation_approach""evaluate.csv"
			done
		done
	done
done
...
do
	for trainset in NIG KEN TAN Sm Sh all_NIG all_COTKEN all_KENTAN all_TANCOT
	do
		python3 evaluate.py $model $trainset "True Prevalence" 0.95 "$folder" "$var" "$val" "$validation_approach" >> "$folder""$var""$validation_approach""evaluate.csv"
	done
done
for model in EnsembleClassifier LogisticRegression ElasticNetLogistic RandomForestClassifier GradientBoostingClassifier SupportVectorClassifier MultilayerPerceptronClassifier
do 
...
do
	...
	do
		python3 score_tables.py "$folder""$var""$validation_approach""evaluate.csv" $metric 1 "$var" >> "$folder""$var""$validation_approach""$metric""_table.tex"
	done
	# F1 and AUC-ROC without CIs
	for metric in F1 AUC
	do
		python3 score_tables.py "$folder""$var""$validation_approach""evaluate.csv" $metric 0 "$var" >> "$folder""$var""$validation_approach""$metric""_table.tex"
	done
done

# generate table 2
python3 best_tables.py "$folder""$validation_approach""evaluate.csv" 1 "" >> "$folder""$validation_approach"best_table.tex

# generate table 3
python3 difference_tables.py "$folder""$validation_approach""evaluate.csv" "$folder""alt_""$validation_approach""evaluate.csv" >> "$folder""$validation_approach""diff_table.tex"

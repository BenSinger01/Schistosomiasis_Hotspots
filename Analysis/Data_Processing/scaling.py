...
,"all_NIG","all_COTKEN","all_KENTAN","all_TANCOT"]:
	data = pd.read_csv(folder+"Data/"+validation_approach+"Train_Data/"+trainset+".csv",index_col=[0])

	X = data[variable_func(trainset)]

	scaler = preprocessing.StandardScaler()
	fit_scaler = scaler.fit(X)
	with open(folder+"Data/"+validation_approach+"Train_Data/"+trainset+"_"+var+"scaler.pickle",'wb') as pklfile:

...
for country in ["NIG","MOZ","COT","KEN","TAN"]:
	...
	train.to_csv(folder+validation_approach+"Train_Data/"+country+".csv")
	test.to_csv(folder+validation_approach+"Test_Data/"+country+".csv")
	if validation_approach=="Fixed":
		...
		fold[val_choice] = 0
		np.savetxt(folder+"FixedTrain_Data/"+country+"_fold.csv",fold,fmt='%1i')


train = all_data[all_data["Country_Code"]=="NIG"]
train.to_csv(folder+validation_approach+"Train_Data/all_NIG.csv")
test = all_data[all_data["Country_Code"]=="MOZ"]
test.to_csv(folder+validation_approach+"Test_Data/all_MOZ.csv")
if validation_approach=="Fixed":
	n_train = train.shape[0]
	val_choice = np.random.choice(range(n_train), size=int(n_train*val_prop/train_prop), replace=False)
	fold = -np.ones(n_train,dtype=int)
	fold[val_choice] = 0
	np.savetxt(folder+"FixedTrain_Data/all_NIG_fold.csv",fold,fmt='%1i')
...
for country1,country2 in [("COT","KEN"),("KEN","TAN"),("TAN","COT")]:
	...
	train.to_csv(folder+validation_approach+"Train_Data/all_"+country1+country2+".csv")
	test = all_data[(all_data["Country_Code"]==country1)]
	test.to_csv(folder+validation_approach+"Test_Data/all_"+country1+".csv")
	if validation_approach=="Fixed":
		n_train = train.shape[0]
		val_choice = np.random.choice(range(n_train), size=int(n_train*val_prop/train_prop), replace=False)
		fold = -np.ones(n_train,dtype=int)
		fold[val_choice] = 0
		np.savetxt(folder+"FixedTrain_Data/all_"+country1+country2+"_fold.csv",fold,fmt='%1i')
...
for species in ["Sh","Sm"]:
	...
	train.to_csv(folder+validation_approach+"Train_Data/"+species+".csv")
	test.to_csv(folder+validation_approach+"Test_Data/"+species+".csv")
	if validation_approach=="Fixed":
		n_train = np.sum(train_ind)
		val_choice = np.random.choice(range(n_train), size=int(n_train*val_prop/train_prop), replace=False)
		fold = -np.ones(n_train,dtype=int)
		fold[val_choice] = 0
		np.savetxt(folder+"FixedTrain_Data/"+species+"_fold.csv",fold,fmt='%1i')

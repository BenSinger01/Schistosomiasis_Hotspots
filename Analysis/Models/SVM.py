...
# select, train, and save support vector models
...
,"all_COTKEN","all_KENTAN","all_TANCOT"]:
	...
	for target in reg_targets:
		...
		if plot:
			...
			plt.savefig(folder+"Models/"+var+val+validation_approach+"Trained_Models/SupportVectorRegressor_"
				+trainset+"_"+target.replace(' ','_')+"_CV_Heatmap.png")
			plt.close()

		# save score
		with open(folder+"Models/"+var+val+validation_approach+"Trained_Models/SupportVectorRegressor_"
	...
	for target in clas_targets:
		...
		if plot:
			...
			ax.set_ylabel(list(search_params.keys())[0])
			ax.set_xlabel(list(search_params.keys())[1])
			plt.savefig(folder+"Outputs/"+var+val+validation_approach+"Trained_Models/SupportVectorClassifier_"
				+trainset+"_"+target.replace(' ','_')+"_CV_Heatmap.png")
			plt.close()

		# save score
		with open(folder+"Outputs/"+var+val+validation_approach+"Trained_Models/SupportVectorClassifier_"
			+trainset+"_"+target.replace(' ','_')+"_score.txt",'w') as wfile:

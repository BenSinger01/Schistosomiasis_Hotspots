...
# test different sizes of feature subset, and try different bootstrap sizes
search_params = {
...
,"all_COTKEN","all_KENTAN","all_TANCOT"]:
	...
	for target in reg_targets:
		...
		if plot:
			...
			plt.savefig(folder+var+val+validation_approach+"Trained_Models/RandomForestRegressor_"
				+trainset+"_"+target.replace(' ','_')+"_CV_Heatmap.png")
			plt.close()
		with open(folder+var+val+validation_approach+"Trained_Models/RandomForestRegressor_"
	...
	for target in clas_targets:
		...
		if plot:
			...
			ax.set_ylabel(list(search_params.keys())[0])
			ax.set_xlabel(list(search_params.keys())[1])
			plt.savefig(folder+var+val+validation_approach+"Trained_Models/RandomForestClassifier_"
				+trainset+"_"+target.replace(' ','_')+"_CV_Heatmap.png")
			plt.close()
		with open(folder+var+val+validation_approach+"Trained_Models/RandomForestClassifier_"
			+trainset+"_"+target.replace(' ','_')+"_score.txt",'w') as wfile:

...
,"all_COTKEN","all_KENTAN","all_TANCOT"]:
	...
	for target in reg_targets:
		...
		if plot:
			...
			plt.savefig(folder+"Outputs"+var+validation_approach+"Trained_Models/MultilayerPerceptronRegressor_"
				+trainset+"_"+target.replace(' ','_')+"_CV_Heatmap.png")
			plt.close()

		# save score
		with open(folder+"Outputs"+var+validation_approach+"Trained_Models/MultilayerPerceptronRegressor_"
	...
	for target in clas_targets:
		...
		if plot:
			...
			ax.set_ylabel(list(search_params.keys())[0])
			ax.set_xlabel(list(search_params.keys())[1])
			plt.savefig(folder+"Outputs"+var+validation_approach+"Trained_Models/MultilayerPerceptronClassifier_"
				+trainset+"_"+target.replace(' ','_')+"_CV_Heatmap.png")
			plt.close()

		# save score
		with open(folder+"Outputs"+var+validation_approach+"Trained_Models/MultilayerPerceptronClassifier_"
			+trainset+"_"+target.replace(' ','_')+"_score.txt",'w') as wfile:

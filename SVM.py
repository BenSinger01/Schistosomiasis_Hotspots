...
# test all regularization values
search_params = {
"C" : 1/np.exp(np.linspace(-10,10,25)),
"gamma" : 1/np.exp(np.linspace(-10,10,25))
}
...
,"all_COTKEN","all_KENTAN","all_TANCOT"]:
	...
	for target in reg_targets:
		...
		if validation_approach=="Fixed":
			fold = np.loadtxt(folder+"FixedTrain_Data/"+trainset+"_fold.csv",delimiter=',',dtype=int)
		...
		if plot:
			...
			score_array = val_results.pivot_table(index="param_C",columns="param_gamma",values="mean_test_score")
			fig,ax=plt.subplots()
			...
			plt.savefig(folder+var+val+validation_approach+"Trained_Models/SupportVectorRegressor_"
				+trainset+"_"+target.replace(' ','_')+"_CV_Heatmap.png")
			plt.close()
		with open(folder+var+val+validation_approach+"Trained_Models/SupportVectorRegressor_"
	...
	for target in clas_targets:
		y = np.array(data[target])
		clas = svm.SVC(probability=True,cache_size=1000)
		if validation_approach=="Fixed":
		...
		if plot:
			...
			ax.set_ylabel(list(search_params.keys())[0])
			ax.set_xlabel(list(search_params.keys())[1])
			plt.savefig(folder+var+val+validation_approach+"Trained_Models/SupportVectorClassifier_"
				+trainset+"_"+target.replace(' ','_')+"_CV_Heatmap.png")
			plt.close()
		with open(folder+var+val+validation_approach+"Trained_Models/SupportVectorClassifier_"
			+trainset+"_"+target.replace(' ','_')+"_score.txt",'w') as wfile:
